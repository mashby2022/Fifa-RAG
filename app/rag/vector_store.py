...
class InMemoryVectorStore(VectorStore):
    ...
    def _lexical_score(query: str, doc: WorldCupDocument) -> float:
        ...
        if isinstance(team, str) and team.lower() in query.lower():
            score += 2.0
        teams = doc.metadata.get("teams")
        if isinstance(teams, list):
            score += sum(0.45 for team_name in teams if isinstance(team_name, str) and team_name.lower() in query.lower())
